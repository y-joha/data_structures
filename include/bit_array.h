...
#include <stdio.h> /* Printf in .c & _test.c*/


#define CHAR_BITS 8 /*you could use <limits.h> in the c file to get the macro -> i'd rather use 2 defines than attach a 
extra header*/
#define SIZE_T_BITS (sizeof(bit_array_t)*CHAR_BITS)
...
size_t BitArrayCountOnMask(bit_array_t ba);
...
size_t BitArrayCountOffMask(bit_array_t ba);
size_t BitArrayCountOffOneLine(bit_array_t ba);
